...
In order to express the rightmost unknown velocity components inside the domain it is required to use the boundary values for $u$ and ghost elements for $v$. The best case scenario would be to have Dirichlet velocity values $\boldsymbol{v}(t,1,y)\subset \mathbb{R}$ or Neumann boundary condition $\frac{\partial u}{\partial x}=0$ as in Colonius-Taira~\cite{Colonius:2008} at the outlet. However, since the behaviour of  solution at the right side of the domain is not known it is not possible to impose such boundary conditions. We will address the spatial discretization at the right boundary in \cref{subsec:laplacian-ABC}.
...
Similarly to Laplacian for the rightmost velocity components in \cref{subsubsec:laplacian-right} we need explicit values of $u$ and  $v$. The velocity values at time steps $n,n-1$ are to be determined in \cref{subsec:advection-ABC}.
