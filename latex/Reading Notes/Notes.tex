...
\begin{enumerate}
  ...
  \item[\textbf{Nonlinear (advect.)}]: Explicit Adams-Bashforth~\eqref{eqn:adams-bashforth} as in Figure~\ref{fig:Advection-discretization}. The term highlighted in orange belongs to the Dirichlet inlet boundary and shall be discarded. On the other hand term highlighted in yellow belongs to the outlet boundary and is unknown, hence it should be kept ($\frac{uu_{ghost}-uu_{rightMost}}{\Delta x}=0$ means $uu_{ghost}=uu_{rightMost}$ which can be used for simplification of the outlet terms). 
...
Let us denote discretization matrices prior scaling with hat symbol ( $\hat{}$ ).
...
Let us have $m$ intervals in $x$ direction and $n$ intervals in $y$ direction. For the case when the face velocities are unknown on a boundary the method words the same way, need to add extra velocities. The number of unknowns is equal to the number of face velocities in the interior of the domain excluding the boundaries. Rewrite $\boldsymbol{u}$ as a vector of unknowns

$$\boldsymbol{u} = [\underbrace{u_1;u_2;\dotsc;u_{m-1};u_1;u_2;\dotsc;u_{m-1};\dotsc;u_1;u_2;\dotsc;u_{m-1}}_{n\text{ times for each }1...m-1\text{ block}}, \underbrace{v_1;v_2;\dotsc;v_{m};v_1;v_2;\dotsc;v_{m};\dotsc;v_1;v_2;\dotsc;v_{m}}_{n-1 \text{ of each }1,\dotsc,m\text{ block}}].$$
...
Now, since $\boldsymbol{q}=R\boldsymbol{u}$ then $\boldsymbol{u}=R^{-1}\boldsymbol{q}$, Going back to Laplacian matrix

$$L\boldsymbol{u}= LR^{-1}\boldsymbol{q},$$ 
