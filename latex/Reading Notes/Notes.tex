...
\text{Noslip BC: } & \boldsymbol{v}(t,x,0)=(0,0) \\
...
\begin{equation}\label{eqn:nse-matrix}
        ...
        \end{equation}
where boundary conditions are in terms of pressure and velocity. In the following subsections, each of the discretization operators is described individually. 

\subsubsection{Laplacian}

Laplacian matrix can be rewritten as blocks
...
Each pair of matrices $\hat{L}_{uxx},\hat{L}_{vxx}$ and $\hat{L}_{uyy},\hat{L}_{vyy}$ will be equal on a uniform, but different on non-uniform grids. These matrices are computed in a similar way. Below, we will discuss how the Laplacian matrix is constructed at different parts of the grid.

\textbf{Inner part.}
Without loss of generality, let us show how to compute $L_{uxx}$, for the other three matrices the distance between each velocity component is different on non-uniform grids, the rest is verbatim. The power series of of $u_{i\pm 1}$ at nodes $x_{i \pm 1}$ with respect to $u_i$ at node $x_i$ are
...
We can combine \eqref{eqn:Taylor left} and \eqref{eqn:Taylor right} by canceling the first derivative, which results in
...
\end{align}
where $h_l=x_i-x_{i-1},h_c = \frac{x_{i+1}-x_{i-1}}{2}, h_r = x_{i+1}-x_i$ and the order of this approximation becomes $2^{\text {nd }}$ for uniform grids. The coefficients in front of the velocity components are put into the corresponding matrix elements.
...
\textbf{Left boundary.} Since the exact value of the left boundary is known in a Boundary Layer problem it is possible to move the corresponding term together with the coefficient to the right-hand side of the linear system and treat the values explicitly. 

\textbf{Top boundary.} 

The top boundary only affects the $\hat{L}_{uyy}$ and $\hat{L}_{vyy}$ due to the 5-point stencil discretization. We will focus on the $\hat{L}_{uyy}$ in this part, the $\hat{L}_{vyy}$ is done similarly with the only exception being the distances between the $v$ velocities due to non-uniform grid. Consider the top most value of $u$ at the $J$'th node with $u_g$ being the ghost velocity
...
The first summand from the above equation is treated explicitly (moved to the rhs), and the other two coefficients are used as elements for the $\hat{L}_{uyy}$ matrix.
...
The coefficients are distributed into the $\hat{L}_{uyy}$ matrix similarly to the boundaries at the left and top of the domain.
...
Let us denote discretization matrices prior to scaling with the hat symbol ( $\hat{}$ ).

Most effective methods of solving a system of linear equations require symmetric matrices, hence, symmetricity is a desired property. It is possible to construct divergence and gradient matrices to contain solely $\pm 1$ coefficients. Let us introduce the diagonal scaling matrix $\hat{M}$ and diagonal flux matrix $R$, s.t.:
\begin{align*}
...
\end{array}\right],
\end{align*}
which results in $q^{n+1}=R\boldsymbol{v}^{n+1}\implies \boldsymbol{v}^{n+1}=R^{-1}q^{n+1}$ and $\hat{D}R^{-1}=-(\hat{M}\hat{G})^T$. More detailed explanation can be found in Appendix~\ref{appendix:symmetric-laplacian}.
...
Publications of Chang~\cite{Chang:2002} and Hall~\cite{Hall:1980} say that in the system~\eqref{eqn:nse-normalized} matrix $D$ is wider than tall, hence it defines a nullspace. The nullspace is the set of all solutions to the homogeneous linear system $Dx = 0$, where $x$ is a vector in the null space. Define the nullspace matrix as $C$. 

The number of rows in the nullspace $C$ is $N_f$, which is the number of faces with unknown velocities. In two dimensions $C$ has $N_n$ columns, which is the number of nodes in the grid, whereas in three dimensions the nullspace has $N_e$ columns, where $N_e$ is the number of edges. 
...
Matrix $C$ is obtained by considering a counterclockwise vorticity around the nodes inside the domain and in some cases the boundary. In case of the open boundary vortices around the nodes belonging to the open segment are taken into consideration. If the direction of the velocity vector on the adjacent face matches the direction of the vorticity then $+1$ is put into the corresponding row, $-1$ in case of the opposite directions of the velocity and vorticity. The matrix $C$ has dimension of unknown velocities (including the open boundary ones) times the number of nodes around which these velocities revolve. Figure~\ref{fig:DC} shows matrices $D$ and $C$ in 4x4 grid case with open boundary on the right side of the domain. Yellow squares indicate $+1$, whereas blue squares correspond to $-1$ entries. The desired product then becomes $DC=0$. 
...
In order to get symmetric diffusion matrix we need to move to the new variables. 
