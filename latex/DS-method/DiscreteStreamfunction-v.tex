...
\begin{figure}[H] % here, bottom, top
  \centering{
  \includegraphics[width=0.55\paperwidth]{ADV}
  }
...
\begin{figure}[H] % here, bottom, top
  \centering{
%  \subfigure[Left]{\includegraphics[width=0.30\paperwidth]{ADV-left}\label{fig:ADV-left}}
  \subfigure[Bottom]{\includegraphics[width=0.30\paperwidth]{ADV-bottom}\label{fig:ADV-bottom}}
  }
...
%\begin{figure}[H] % here, bottom, top
%  \centering{
%  \includegraphics[width=0.60\paperwidth]{ADV-top-right}
%  }
...
\begin{figure}[H] % here, bottom, top
  \centering{
  \includegraphics[width=0.45\paperwidth]{G-example-2x2}
  }
...
\begin{figure}[H] % here, bottom, top
  \centering{
  \includegraphics[width=0.35\paperwidth]{Luxx-bottom}
  }
...
\begin{enumerate}
	...
	\item Rewrite $q^{n+1}=q^{n+1}_h+q^{n+1}_p$.\label[step]{step:2}
	\item Precompute $bc_1^n$, $r^n$ and $q^{n+1}_p$. \label[step]{step:3}
	\item Eliminate pressure terms in the momentum equation.\label[step]{step:4}
		\begin{align}
			Aq^{n+1}&=-Gp^{n+1}+r^n+bc_1^n,\notag\\
			A(q^{n+1}_h+q^{n+1}_p)&=D^Tp^{n+1}+r^n+bc_1^n, &\text{ premultiply by $C^T$,} \notag\\
			C^TAq^{n+1}_h&=C^T(r^n+bc_1^n-Aq^{n+1}_p), &\text{ use $q^{n+1}_h=C\psi^{n+1}$,}\notag\\
			C^TAC\psi^{n+1}&=C^T(r^n+bc_1^n-Aq^{n+1}_p)\label[system]{eqn:resulting-system}.
		\end{align}
	...
	\item Compute $q^{n+1}=q^{n+1}_h+q^{n+1}_p$ and convert it to velocity vector field $\boldsymbol{v}^{n+1}=R^{-1}q^{n+1}$.\label[step]{step:7}
	\item Transition to next time instance by repeating \cref{step:3,step:4,step:5,step:6,step:7}.
\end{enumerate}

\pagebreak
\section{Results}
Simplest boundary condition case with $Re=100$, $(u,v)=(0,0)$ at the wall, $(u,v)=(1,0)$ both at inlet, outlet and freestream produced vector fields displayed in \cref{fig:simplest-bc}.

\begin{figure}[H] % here - h, bottom - b, top - t
  \centering{
  \includegraphics[width=0.85\paperwidth]{Dirichlet_BC}
  }
  \caption{}\label{fig:simplest-bc}
\end{figure}
