...
% particular solution
% using Pinv
qp = pinv(D)*boundary2(M, N, dx, dy, q, u, v,dt,1,x,Amplitude,k,omega,uInlet);    % using PINV
...
%     recalculate particular solution
    ctr = 1;
    uparticular  = zeros(1,nu);
    for i = 1:N
...
%     pbc = LBC;
    ...
    advPrev = advection(M,N,dx,dy,u,v,timeIndex-1,dt,uInlet);
...
%         vplot = vplot(:,1:M);
        ...
        sumleft = -sum(abs(boundary2(M, N, dx, dy, q, u, v,dt,1,x,Amplitude,k,omega,uInlet)));
        sumright = sum(abs(q(M:M:nu,timeIndex)));
        sumtop = sum(abs(q(nq-M+1:1:nq,timeIndex)));
        fprintf('massFluxBdry: L=%18.15f, R=%18.15f, T=%18.15f, sum=%+.6e',sumleft,sumright,sumtop,sumleft+sumright+sumtop);
...
%         sih = quiver(X,flipud(Y),(uploth)',(vploth)',0);
        ...
        colorbar;
        si = streamslice(X,flipud(Y),uplot',vplot',15);
        ...
        nexttile(3,[1 1]);
        contourf(X,flipud(Y),(uplot.^2)',50,'LineColor','none'); hold on;
        colorbar
        title(sprintf('u velocity contour at %3d''th time step',timeIndex));

        nexttile(6,[1 1]);
        contourf(X,flipud(Y),(vplot.^2)',50,'LineColor','none'); hold on;
        colorbar
...
%         % plot bc left
        nexttile(9,[3,1]);
        plot([uInlet(:,timeIndex)]',0.5*(y(1:end-1)+y(2:end)));
        title(sprintf('inlet u component BC at %3d',timeIndex));

%         % plot bc right
        nexttile(12,[3,1]);
        plot([u(M:M:nu,timeIndex)]',0.5*(y(1:end-1)+y(2:end)));
...
%     figure;
    ...
    for iu = 1:nu
        ...
        if col==M
            ...
            uuRightDiff = 0.25*abs(ug+uprev(iu))*(ug-uprev(iu)); 
            duudx(iu) = (uuright - uu(leftcell) - 1*(uuRightDiff - uHorAvg_uHorDiff(leftcell)))/(deltax);
        else
...
%         fprintf('r:%3d, c:%3d, botcell=%3d, topcell=%3d, leftnode=%3d, rightnode=%3d',row,col,botcell,topcell,leftnode,rightnode);
        ...
        if row==N
            ...
            vvtop = vprev(iv)*vprev(iv); % fprintf('%18.15f\n',vvtop); % backward upwind difference
