...
phi = pi/2;
Amplitude = .5;
targetValue = 0.5;       % Desired target Amplitude value
...
U0 = 1+Amplitude;             % characteristic velocity scale
...
function [u, v, q] = initialConditions(M, N, nt, dyq, dxq)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %       Initialize u, v, and q with initial conditions              %
    %       Adjusted to exclude v at the top boundary                   %
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

    nu = M * N;
    nv = M * (N - 1);  % Adjusted nv
    nq = nu + nv;
    ...
    % Initialize u with a uniform profile (or as needed)
    u_initial = ones(nu, 1);  % Modify as per your initial condition
    u(:, 1:3) = repmat(u_initial, 1, 3);

    % Initialize v to zero (since v = 0 at top boundary)
    v_initial = zeros(nv, 1);
    v(:, 1:3) = repmat(v_initial, 1, 3);

    % Compute initial q
    q(1:nu, 1:3) = u(:, 1:3) .* dyq';
    q(nu + 1:end, 1:3) = v(:, 1:3) .* dxq';
end
...
function [dxq, dyq] = delQ(M, N, dx, dy)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %       delQ computes dyq and dxq arrays for q = [q_u; q_v]         %
    %       Adjusted to exclude v at the top boundary (v = 0 there)     %
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % dyq for u components
    for i = 1:N
        idx_start = (i - 1) * M + 1;
        idx_end = i * M;
        dyq(idx_start:idx_end) = dy(i);
    end

    % dxq for v components (exclude top boundary)
    for i = 1:(N - 1)
        idx_start = (i - 1) * M + 1;
        idx_end = i * M;
        dxq(idx_start:idx_end) = dx;
    end
end
...
function [dvudx, dvvdy] = computeDvudxDvvdy(M, N, dx, dy, uprev, vprev, vv, vVerAvg_vVerDiff, uv, uVerAvg_vHorDiff, gamma, duFreestreamdx, timeIndex)
    ...
    for iv = 1:nv
        ...
        if row == N
            ...
            % Compute du/dx at the top boundary
            du_dx = duFreestreamdx(col, timeIndex);  % Ensure correct time index
            % Compute dv/dy using continuity
            dv_dy = -du_dx;
            % Estimate vg using central difference
            dy_top = dy(row);
            v_top = vprev(iv);
            vg = v_top + (2* dy_top) * dv_dy;
            % Now compute vvtop and vvTopDiff
