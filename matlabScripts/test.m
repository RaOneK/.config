...
dt = 1e-5;          % desired timestep size
Re = 122500;           % Reynolds number
...
%     compute inlet 
    ...
    uFreestream(:,timeNow) = uFree(x,Amplitude,k,omega,timeI,phi); 
...
%         title(sprintf('sum of mass fluxes in each cell'));
        ...
        % Plot the particular solution (u and v components combined)
%         contourf(X, flipud(Y), (u_particular.^2 + v_particular.^2)', 50, 'LineColor', 'none'); 
        
        
        % Overlay streamlines to visualize flow
        sih = streamslice(X,flipud(Y),(uplotp'),(vplotp'),'noarrows'); 
        set(sih, 'Color', 'red');
...
%         colorbar
        s=surf(flipud(X),flipud(Y),(uplot'));
        s.EdgeColor = 'none';
        set(si,'Color','red');
        title(sprintf('u surf. Back left = freestream, front left = inlet'));

        nexttile(13,[2 2]);
        s=surf(X,flipud(Y),(vplot')); hold on;
        s.EdgeColor = 'none';
        colorbar
        title(sprintf('v surf. Back left = freestream, front left = inlet'));
...
function [duFreestream] = duFree(Amplitude,k,omega,time,x,phi)
    duFreestream = -k*Amplitude*sin(omega*time + k*0.5*(x(2:end)+x(1:end-1))+phi); % actually its -du/dx
end
