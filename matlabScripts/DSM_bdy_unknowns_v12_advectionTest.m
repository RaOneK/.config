...
timeMaxIter = 4;  % number of timesteps
...
qp = generateParticularSolution(M, N, dxq, dyq, uInlet);
...
%         intermediateSteps = 0;
        ...
        sih = streamslice(X,flipud(Y),(uploth'),(vploth'),'noarrows'); 
%         contourf(((vploth).^2 + (uploth).^2)','LineColor','none'); hold on; colorbar;
%         sih = streamslice((uploth'),(vploth'),'noarrows'); 
...
%         set(sip,'Color','red');
        ...
        contourf(X,flipud(Y),(uplot.^2+vplot.^2)','LineColor','none'); hold on;
...
%         HORIZ = plot(X.',Y.','LineWidth',0.0625,"Color", [0.4, 0.4, 0.4, 1]); hold on; 
        ...
        nexttile(1,[2 2]);
%         contourf(X(2:end,2:end),flipud(Y(2:end,2:end)),vorticity','LineColor','none'); hold on;
%         si=streamslice(X(1:floor(end/8),1:end),flipud(Y(end-floor(end/8)+1:end,1:end)),(uplot(1:end,1:floor(end/8)))',(vplot(1:end,1:floor(end/8)))','noarrows'); hold on;
%         colorbar
        surf(flipud(X),flipud(Y),flipud(uplot'));
        set(si,'Color','red');
        title(sprintf('u surf. Back left = wall, front left = inlet'));

        nexttile(13,[2 2]);
        surf(X,flipud(Y),flipud(vplot')); hold on;
        colorbar
        title(sprintf('v surf. Back left = wall, front left = inlet'));
...
%             vg = -2*deltay*(uRprevprev - uLprevprev)/deltax + vprev(iv-M);
            ...
            vvtop = 0.25*(-vprev(iv)+vprev(iv))^2; 
            vvTopDiff = 0.25*abs(-vprev(iv)+vprev(iv))*(vg-vprev(iv)); 
...
function [qp] = generateParticularSolution(M, N, dxq, dyq, uInlet)
    % This function generates a particular solution (qp) based on the grid 
    % dimensions M, N, the grid spacing dxq, dyq, and the inlet profile uInlet.
    
    nu = M * N;        % Number of velocity components in x-direction
    nq = 2 * nu;       % Total number of components (u and v)

    % Initialize qp
    qp = zeros(nq, 1);

    % Construct u-particular (horizontal velocity) based on uInlet (inlet profile)
    ctr = 1;
    uparticular = zeros(1, nu);
    
    % Loop through each row (for uInlet) and assign velocities for each cell in x-dir
    for i = 1:N
        uparticular(ctr:ctr + M - 1) = uInlet(i); % Apply the inlet profile for each row
        ctr = ctr + M;                            % Move to the next row
    end
    
    % Apply to qp: the horizontal velocity part of qp
    qp(1:nu) = dyq .* uparticular;  % dyq accounts for the spacing in y-direction
    
    % For vertical velocity, assume uniform horizontal flow with no vertical component
    qp(nu + 1:nq) = dxq .* (0.0);   % Set vertical velocity part to zero
