...
stepPlot = 250;
M = 26;      % grid size in x-dir
N = 26;      % grid size in y-dir
nt =1e5;     % number of timesteps
dt = 5e-4;  % timestep size
Re = 9500;   % Reynolds number
...
% ^ DONE
function [ADV] = advection(M,N,dx,dy,u,v,timeIndex) % inlet at left needs to be hardcoded (uf)
    nu = (M)*N;
    nv = M*(N);
    uprev = u(:,timeIndex);
    vprev = v(:,timeIndex);
