...
%         fprintf("R: %3d C: %3d\n",row,col);
        ...
        elseif col == M
            ...
            he = dx(M);
            hc = 0.5*(he+hw);
            Luxx(iu,iu) = 1/(hw*hc); % new DONG BC
...
%         fprintf("R: %3d C: %3d\n",row,col);
        ...
        elseif row == N
            ...
            Luyy(iu,iu) = -2/(hs*hn)-1/(hn*hc); % keep tangential u at top uf=0.5(ug+utopmost)
            Luyy(iu,iu) = -1/(hs*hc); % DONG BC
...
%         uf(idx) = 1+acos;
%         LBC(iu) = LBC(iu)+2*uf(idx)/(hn*hc); % shear tangential velocity
%         LBC(iu) = LBC(iu)-2*uf(idx)/(hn*hc); % dong bc du/dy at top = 0
        % normal component
        hc = dy(N);
        deltax = dx(idx);
