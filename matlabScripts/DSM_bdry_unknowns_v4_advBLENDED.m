...
blendingFactor = 0.85;
stepPlot = 50;
M = 20;      % grid size in x-dir
N = 20;      % grid size in y-dir
nt =500001;     % number of timesteps
...
check_flag = 0;
kx = 1.03;  % grid ratio in x-dir
ky = 1.03;  % grid ratio in y-dir
% kx = 1;
% ky = 1;
...
for timeIndex = 4:nt
    ...
    [ADV] = 1.5*advection(M,N,dx,dy,u,v,timeIndex-1,blendingFactor)-0.5*advection(M,N,dx,dy,u,v,timeIndex-2,blendingFactor);
...
%     v(u > 1) = 1;
    ...
    if (mod(timeIndex,stepPlot)==0) || (timeIndex==3)
        ...
        contourf(X,flipud(Y),(uplot),50,'LineColor','none'); hold on; colorbar;
%         contourf(X,flipud(Y),(vploth.^2)', 20,'b--'); hold on;
%         contourf(X,flipud(Y),(uploth.^2+vploth.^2)',50,'LineColor','none'); hold on; colorbar;
...
% below uplothomogeneous magnitude
%         contourf(X,flipud(Y),(uplotp.^2+vplotp.^2)',50,'LineColor','none'); hold on; colorbar;
        contourf(X,flipud(Y),(vplot),50,'LineColor','none'); hold on; colorbar;
...
%         plot(x,pbc(nq-M:1:nq),'blue');
        hold on;
        plot((x(2:end)+x(1:end-1))/2,v(nv-M+1:1:nv,timeIndex),'red');
        title(sprintf('top velocity BC at %3d',timeIndex));

%         % plot bc left
        nexttile(9,[3,1]);
        plot([u(M:M:nu,timeIndex)]',(y(1:N)+y(2:N+1))/2);
...
% ^ DONE
function [ADV] = advection(M,N,dx,dy,u,v,timeIndex,blendingFactor) % inlet at left needs to be hardcoded (uf)
    nu = (M)*N;
    nv = M*(N);
    uprev = u(:,timeIndex);
    vprev = v(:,timeIndex);
...
%         fprintf('row: %3d, col: %3d, L:%4d, R:%4d, B:%4d, T:%4d\n',row,col,ijleft,ijright,ijbot,ijtop);
        % compute uu 
        if col == 1 % INLET
            uu(iCell) = 0.25* (1+uprev(ijright))^2; 
...
%         fprintf('row: %4d, node: %4d\n',row,iuv);
        ...
        elseif (row==N+1) && (col~=M+1) % du/dy=0
            ...
            uv(iuv) = uprev(ijbot)*(he*vprev(ijleft) + hw*vprev(ijright))/(he+hw);
...
%         fprintf('r:%3d, c:%3d, botcell=%3d, topcell=%3d, leftnode=%3d, rightnode=%3d',row,col,botcell,topcell,leftnode,rightnode);
        ...
        if row == N
            ...
            if col>1
                ...
                utopleft = 1; % freestream
                ubotleft = 1;
            end
        else
            utopright = uprev(iv+M);
            ubotright = uprev(iv);
            if col>1
                utopleft = uprev(iv+M-1);
                ubotleft = uprev(iv-1);
            else
                utopleft = 1; % freestream
                ubotleft = 1;
        ...
        CDS = (uv(rightnode) - uv(leftnode))/deltax;
        dvudx(iv) = CDS;
        if row==N
            vvtop = vv(botcell);
            vvtop = vprev(iv)*vprev(iv);
            CDS = (vvtop - vv(botcell))/deltay;
            dvvdy(iv) = CDS;
%             fprintf('\t %3d. vvtop = %18.15f, vvbot = %18.15f, dy=%18.15f, vg = %18.15f\n', iv, vvtop,vv(botcell),deltay,vg);
            ...
            UDS = max(vc,0) * (vc - vbot)/dy(row) + min(vc,0)*(vc-vtop)/dy(row+1);
            CDS = (vv(topcell) - vv(botcell))/deltay;
            dvvdy(iv) = blendingFactor*CDS + (1-blendingFactor)*UDS;
