...
dt = 5e-6;          % desired timestep size
Re = 4500;           % Reynolds number
...
kx = 1;
ky = 1;
lx = 0.05;     % x dimension scale
ly = 0.01;     % y dimension scale
...
for t=1:3
    uInlet(:,t) = inletProfile(y,Amplitude,omega,t*dt,N);
...
%     compute inlet 
    uInlet(:,timeNow) = inletProfile(y,Amplitude,omega,time,N);
...
%         intermediateSteps = 0;
        ...
        if (any(isnan(uminNow))) || (max(abs(u(:,timeNow)))>10)
            fprintf('\n\n Solution blew up \n\n');
            close all;
            return
        end
...
%         HORIZ = plot(X.',Y.','LineWidth',0.0625,"Color", [0.4, 0.4, 0.4, 1]); hold on; 
        ...
        nexttile(1,[2 2]);
%         contourf(X(2:floor(end/8),2:end),flipud(Y(end-floor(end/8)+2:end,2:end)),(vorticity(1:end,2:floor(end/8)))','LineColor','none'); hold on;
        ...
        colorbar
        set(si,'Color','red');
        title(sprintf('streamlines and vorticity contour at the %d Length corner',floor(N/8)));

        nexttile(13,[2 2]);
        upp = qprev(1:nu)./dyq';
        upp = reshape(upp,M,N);
        contourf(X,flipud(Y),(upp)','LineColor','none'); hold on;
        colorbar
        ...
        title(sprintf('inlet u component BC=%8.5f',sum(uInlet(:,timeNow))/N));
...
function [uInlet] = inletProfile(y,A,w,time,N)
    uInlet=(1+A*cos(w*time)) + 2e-8 * (0.5 - rand(1, N));%e*[tanh(smootheningFactor*(0.5*(y(1:end-1)+y(2:end))))];
    uInlet(1) = uInlet(1)/2;
end
